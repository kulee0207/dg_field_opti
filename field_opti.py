...
def compute_coverage_with_horizontal(H_s, H_r, D, beamwidth_v_deg, beamwidth_h_deg, show_plot=True):
    ...
    if show_plot:
        ...
        ax.plot(-tilt_range, coverages)
        ax.axvline(-best_data["최적 틸트 각도"], color='red', linestyle='--', label=f"Best Tilt = {-best_data['최적 틸트 각도']:.0f}°")
        ax.axhline(best_data["최대 수직 커버리지"], color='blue', linestyle='--', label=f"Max Verticla_Coverage = {best_data['최대 수직 커버리지']:.2f}°")
        ax.set_title("틸트각에 따른 수직 커버리지 길이")
        ax.set_xlabel("틸트 각 (°)")
        ax.set_ylabel("수직 커버리지 (m)")
        ax.set_ylim(bottom=0)
        ax.set_xlim(-tilt_range_valid[0],-tilt_range_valid[-1])
        ax.grid(True)
        ax.legend()
        fig.tight_layout()
        plt.show()
